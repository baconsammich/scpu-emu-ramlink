...
**U5. Does `PLB` wrap inside page 1 in emulation mode?** The same shape as U1,
found the same way -- by running the full suite rather than the subset.

`PLB` at `S=$01FF` pre-increments and reads. Wrapped that is `$000100`; unwrapped
it is `$000200`. Either way `S` ends at `$0100`, so nothing but the *address* of
the read distinguishes them, which is why a register-only comparison misses it
entirely.

- *Wrap*: WDC S1 §8.1 scopes its exception list to opcodes "accessing two or
  three bytes", so a one-byte pull falls under the general rule that the
  emulation stack range is `000100` to `0001FF`. VICE's `PLB(LOAD_STACK)` goes
  through `PULL()`, which wraps.
- *No wrap*: S4 `ab.e`, 37/37 boundary cases read `$000200`.

**Decision: wrap**, on the U1 precedent -- WDC plus VICE against one
reimplementation. Pinned by
`w65c816_emulation_plb_pull_crosses_out_of_page_one`.

**U6. Does `JSR (a,X)` run off the bottom of page 1?** U5 inverted: here the
datasheet is *explicit* and it is S4 that departs from it.

- *No wrap (new class)*: S1 §8.1 names `JSR (a,x)` in the list outright. VICE's
  `JSR_IND_X()` stores the first byte at `reg_sp`, then does a bare 16-bit
  `reg_sp--` before the wrapping `PUSH` -- deliberately not `PUSH` twice, which
  is what `JSR()` immediately above it does.
- *Wrap (old class)*: S4 `fc.e`, 87/87 boundary cases stay inside page 1,
  identical to `JSR abs`.

**Decision: no wrap**, same reasoning. Pinned by
`w65c816_emulation_jsr_abs_x_runs_off_the_bottom_of_page_one`.

> **Scope of the three deviations.** The full suite is 5,080,000 vectors across
> all 256 opcodes in both modes. Every one of the 80 disagreements is U1, U5 or
> U6; there is no fourth. `Tools/viceconf/ss816.cpp` carries the list with the
> expected count per opcode, so a clean run is green and a *changed* count is a
> regression.
...
`PHB` and `PHK` push one byte and cannot cross: a push writes at `S` and *then*
decrements, so it never leaves the page whatever its class. **`PLB` is not in
that category** -- an earlier version of this paragraph said it was, and it is
wrong. A pull PRE-increments and then reads, so `PLB` at `S=$01FF` reads either
`$000100` or `$000200` depending on its class, and the difference is invisible
in the final `S` (`$0100` either way). See **U5**.
