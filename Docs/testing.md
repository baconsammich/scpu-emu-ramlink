...
Over 650,000 checks, all running on the host with no hardware. The bulk of
...
For memory-safety and undefined-behaviour checks, run:

```sh
make sanitizers
```

`make check` runs both the normal optimized suite and the sanitizer build.
