...
- [x] REU coexistence, or reuse of RAD's REU emulation alongside the accelerator
- [x] **CMD RAMLink.** The device: register file, the `$DE00` window onto either
      its internal 8K or the RAMCard, the RAMCard size decode (which is how
      software sizes the card, and is a table rather than a mask for exactly
      that reason), the RTC, and the RAM-Port REU trap. Written against VICE's
      `ramlink.c` and pinned by `Tests/RAMLink/`. The DOS ROM is deliberately
      not banked into `$8000-$BFFF`: VICE declines that mapping for SCPU64
      because the accelerator's own DOS carries RAMLink support, which is why
      `$D0BC` bit 6 and the vector reroute already knew about RAMLink. See
      [RAMLink.md](RAMLink.md).
- [ ] RAMCard persistence across a power cycle. The Pi has no battery, so a
      partitioned card comes up blank; saving an image needs the FAT driver,
      which is released before `/DMA` is asserted.
- [ ] A CMD HD on RAMLink's parallel bus. The i8255A is present and a probe
      fails cleanly, which is correct with no drive attached.
