...
case "$(uname -s)" in
	...
	Linux)
		...
		TOOLCHAIN_KIND="tar" ;;
	Darwin)
		# ARM ships a separate macOS build per CPU, and the Apple-silicon one
		# will not execute on an Intel Mac -- it fails at the first compiler
		# invocation with "Bad CPU type in executable", a long way from here.
		# Select on uname -m rather than assuming Apple silicon.
		case "$(uname -m)" in
			arm64|aarch64)
				TOOLCHAIN_FILE="arm-gnu-toolchain-13.2.rel1-darwin-arm64-aarch64-none-elf.tar.xz" ;;
			*)
				TOOLCHAIN_FILE="arm-gnu-toolchain-13.2.rel1-darwin-x86_64-aarch64-none-elf.tar.xz" ;;
		esac
		TOOLCHAIN_SHA=""
		TOOLCHAIN_KIND="tar" ;;
	*)
...
# Parallelism. nproc is coreutils and is absent on macOS; sysctl is the BSD
# equivalent. Fall back to 4 rather than failing, since this only sets -j.
_ncpu() {
...
if [ ! -f "$CIRCLE/lib/libcircle.a" ] \
   || [ "$CIRCLE_SETTINGS_SHA" != "$OLD_CIRCLE_SETTINGS_SHA" ]; then
	...
	( cd "$CIRCLE/lib"          && make -j"$(_ncpu)" )
	( cd "$CIRCLE/lib/fs"       && make -j"$(_ncpu)" )
	( cd "$CIRCLE/lib/sched"    && make -j"$(_ncpu)" )
	( cd "$CIRCLE/addon/SDCard" && make -j"$(_ncpu)" )
	( cd "$CIRCLE/addon/fatfs"  && make -j"$(_ncpu)" )
	( cd "$CIRCLE/addon/linux"  && make -j"$(_ncpu)" )
...
if [ -f "$CIRCLE/Source/Firmware/kernel8.img" ]; then
	...
	# Count with a glob rather than `ls ... | wc -l`. Under `set -o pipefail`
	# that pipeline FAILS when the directory is empty -- ls exits 2 on no match
	# -- so `set -e` aborted the script on the very first build, after
	# kernel8.img had already been produced and copied. The build looked broken
	# and was not.
	N=0
	for f in "$REPO/build/kernels"/kernel8-*.img; do
		[ -e "$f" ] && N=$((N + 1))
	done
	N=$((N + 1))
