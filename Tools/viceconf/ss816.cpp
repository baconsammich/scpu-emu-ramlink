...
// --- known deviations -------------------------------------------------------
// Three emulation-mode encodings where this core deliberately does NOT match
// SingleStepTests. Each is a documented conflict between the suite and the two
// sources that outrank it -- the WDC datasheet and VICE -- and each is pinned
// by a unit test so the decision cannot drift silently. See
// Docs/SuperCPU64/65816-reference.md sections U1, U5 and U6.
//
// The point of listing them here is that a conformance run is only useful as a
// gate if a clean run is green. Before this table the full set reported 80
// failures every time, which is indistinguishable from a real regression and
// trains everyone to ignore the output. A deviation that exceeds its expected
// count is still reported as a failure: the exemption is for a known quantity
// of known cases, not a blanket pardon for the opcode.
struct KnownDeviation
{
	long        opcode;
	char        mode;		// 'e' emulation, 'n' native
	u32         expected;	// how many of the 10,000 vectors are expected to differ
	const char *why;
};

static const KnownDeviation g_KnownDeviations[] =
{
	{ 0xAB, 'e', 36, "U5 PLB pull wraps inside page 1 (WDC+VICE) vs crosses (suite)" },
	{ 0xE1, 'e',  1, "U1 direct-page pointer high byte wraps (WDC+VICE) vs not (suite)" },
	{ 0xFC, 'e', 43, "U6 JSR (a,X) crosses page 1 (WDC+VICE) vs wraps (suite)" },
};

static const KnownDeviation *findDeviation( long opcode, char mode )
{
	for ( unsigned i = 0;
	      i < sizeof g_KnownDeviations / sizeof g_KnownDeviations[ 0 ]; i++ )
		if ( g_KnownDeviations[ i ].opcode == opcode
		     && g_KnownDeviations[ i ].mode == mode )
			return &g_KnownDeviations[ i ];
	return 0;
}
...
static u32 failures = 0, passes = 0, skipped = 0, deviations = 0;
...
int main( int argc, char **argv )
{
	...
	for ( int fi = first; fi < argc; fi++ )
	{
		...
		// "<op>.<mode>.json" -- the mode letter sits after the first dot.
		char mode = 'n';
		for ( const char *q = base; *q; q++ )
			if ( *q == '.' && q[ 1 ] ) { mode = q[ 1 ]; break; }
		const KnownDeviation *dev = findDeviation( opcode, mode );
		...
		while ( cur < end )
		{
			...
			#define CHECK(field,label,got) \
				if ( (long)(got) != (long)(field) ) \
					{ if ( !bad && !dev ) report( name, label, (long)(field), (long)(got) ); \
					  bad = true; }
		...
		else if ( dev && fileFail <= dev->expected )
		{
			deviations += fileFail;
			fprintf( stderr, "%-24s %6u tests, %6u deviate  ok  (known: %s)\n",
			         argv[ fi ], fileRun, fileFail, dev->why );
		}
		else
		{
			if ( dev )
				fprintf( stderr, "  %s: expected at most %u known deviations, "
				                 "got %u -- treating as a regression\n",
				         argv[ fi ], dev->expected, fileFail );
			failures += fileFail;
	...
	fprintf( stderr, "\n%u passed, %u failed, %u skipped, "
	                 "%u known deviations\n",
	         passes, failures, skipped, deviations );
	return failures ? 1 : 0;
}
