...
static u8 *loadDump( const char *path, u32 expected, u32 &size )
{
	...
	long total = ftell( f );
	if ( total < 2 || (unsigned long)total > 0xFFFFFFFFUL
	     || fseek( f, 0, SEEK_SET ) != 0 )
	{
		fprintf( stderr, "invalid dump size for %s\n", path );
		fclose( f );
		return 0;
	}
	...
	{
		fprintf( stderr, "cannot seek past header in %s\n", path );
		fclose( f );
		return 0;
	}
	size = (u32)( total - skip );
...
int main( int argc, char **argv )
{
	...
	bool outputOK = fwrite( pixels, 1, sizeof pixels, out ) == sizeof pixels;
	if ( fclose( out ) != 0 ) outputOK = false;
	if ( !outputOK )
	{
		fprintf( stderr, "short or failed write to %s\n", argv[ 4 ] );
		free( ram ); free( io ); free( rom );
		return 1;
	}
