...
**The snapshot itself is not in this repository.** A VICE `.vsf` captures the
whole machine, which includes Commodore's BASIC and KERNAL ROM images, so it
is not redistributable and was removed from the tree and from history before
this repository was made public.

These tools still work; you supply your own snapshot. Take one in VICE
SCPU64 at whatever moment you are investigating (the original was 3D Pool
/SCPU's table screen) and point the scripts at it.
