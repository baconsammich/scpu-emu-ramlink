...
TEST( w65c816_emulation_plb_pull_crosses_out_of_page_one )
{
	// U5, pinned. PLB with S=$01FF reads $000200, not $000100.
	//
	// The reference doc's old/new table says "PHB, PHK and PLB push one byte
	// and cannot cross, so their class does not matter." That is true of the
	// two PUSHes and false of the PULL: a push writes at S and then decrements,
	// so it cannot leave the page, but a pull PRE-increments and then reads, so
	// at S=$01FF it reads $0200 unless something wraps it.
	//
	// Sources split, and the split is the whole reason this test exists:
	//   - WDC S1 section 8.1 lists only instructions "accessing two or three
	//     bytes", so PLB falls under the general "stack range is 000100 to
	//     0001FF" rule -- wrap.
	//   - VICE's PLB(LOAD_STACK) goes through PULL(), which wraps.
	//   - SingleStepTests ab.e disagrees: 37/37 boundary cases read $000200 and
	//     end with S=$0100.
	// Two sources against one reimplementation, so we wrap -- the same call
	// made for U1, for the same reason. Note the final S is $0100 either way,
	// which is what makes the difference invisible to a register-only check.
	const u8 code[] = {
		0xA2, 0xFF,				// LDX #$FF
		0x9A,					// TXS      S = $01FF
		0xAB					// PLB
	...
	W65Fixture f( code, sizeof( code ) );
	f.run( 2 );
	CHECK_EQ( f.cpu.m_S, 0x01FF );

	s_Mem[ 0x000100 ] = 0x5A;		// wrapped source
	s_Mem[ 0x000200 ] = 0xA5;		// unwrapped source
	f.bus.clearLog();
	f.cpu.step();

	CHECK( f.bus.readAddress( 0x000100 ) );
	CHECK( !f.bus.readAddress( 0x000200 ) );
	CHECK_EQ( f.cpu.m_DBR, 0x5A );
	CHECK_EQ( f.cpu.m_S, 0x0100 );
}

TEST( w65c816_emulation_jsr_abs_x_runs_off_the_bottom_of_page_one )
{
	// U6, pinned. JSR (abs,X) with S=$0100 writes $000100 then $0000FF.
	//
	// This one is the mirror image of U5: here the datasheet is explicit --
	// S1 section 8.1 names "JSR (a,x)" in the list of opcodes that increment or
	// decrement beyond page 1 -- and VICE's JSR_IND_X() agrees, storing the
	// first byte at reg_sp and then doing a bare 16-bit reg_sp-- before the
	// wrapping PUSH. SingleStepTests fc.e disagrees on all 87 boundary cases,
	// wrapping inside page 1 exactly as JSR abs does.
	//
	// Same verdict, same reasoning: WDC plus VICE outrank the reimplementation.
	// Kept next to the JSR abs test because the ONLY difference between them is
	// this, and a regression would silently make the two identical.
	const u8 code[] = {
		0xA2, 0x00,				// LDX #$00
		0x9A,					// TXS      S = $0100
	...
	s_Mem[ 0x003000 ] = 0x00;		// jump vector -> $4000
	s_Mem[ 0x003001 ] = 0x40;
	f.bus.clearLog();
	f.cpu.step();
	...
	CHECK_EQ( addr, 0x000100 );
	CHECK( f.bus.nthWrite( 1, addr, value ) );
	CHECK_EQ( addr, 0x0000FF );		// NOT $0001FF -- that would be JSR abs
	CHECK_EQ( f.cpu.m_S, 0x01FE );
