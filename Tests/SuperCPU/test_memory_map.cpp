...
TEST( scpu_dos_extension_maps_its_bank1_work_areas_for_reads_only )
{
	CHostBus bus;
	CC64Memory bank0;
	CSuperCPUMemoryMap map;
	...
	// The mapping is READ-ONLY. VICE's R1 entry installs a read function and
	// a read base and leaves the write hook at its preset, which is ram_store
	// -- bank 0. So a store through one of these windows lands in ordinary
	// RAM and bank 1 is untouched.
	//
	// This was pinned the other way round, and bank 1 $8000-$9FFF is where
	// RL-DOS actually lives: CMD's DOS writes to $8000-$9FFF as ordinary RAM
	// all through its init, so routing those writes to bank 1 dismantled the
	// code it was running. Measured as bank 1 falling from 8192/8192 matching
	// the ROM to 1555/8192, then a spin loop in the rubble.
	bank0.writeFast( 0x1234, 0xC3 );
	bank0.writeFast( 0x8123, 0xD4 );
	CHECK_EQ( map.m_Bank1[ 0x1234 ], 0xA1 );
	CHECK_EQ( map.m_Bank1[ 0x8123 ], 0xB2 );
	CHECK_EQ( bank0.m_RAM[ 0x1234 ], 0xC3 );
	CHECK_EQ( bank0.m_RAM[ 0x8123 ], 0xD4 );

	// Reads still come from bank 1 while the extension is on...
	CHECK_EQ( bank0.readFast( 0x1234 ), 0xA1 );
	CHECK_EQ( bank0.readFast( 0x8123 ), 0xB2 );

	// ...and the bank-0 bytes the writes actually made are there afterwards.
	regs.ioWrite( SCPU_REG_DOSEXT_OFF, 0 );
	CHECK_EQ( bank0.readFast( 0x1234 ), 0xC3 );
	CHECK_EQ( bank0.readFast( 0x8123 ), 0xD4 );
}
