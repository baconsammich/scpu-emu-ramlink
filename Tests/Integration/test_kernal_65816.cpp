...
static bool load816ROM( const char *logical, u8 *dst, u32 expected )
{
...
struct Machine816
{
	...
	Machine816()
	{
		...
		haveROMs = load816ROM( "kernal.rom", kernal, C64_KERNAL_SIZE )
		        && load816ROM( "basic.rom",  basic,  C64_BASIC_SIZE );
		if ( !haveROMs ) return;

		mem.setKernalROM( kernal );
		mem.setBasicROM( basic );
		if ( load816ROM( "chargen.rom", chargen, C64_CHARROM_SIZE ) )
