...
static bool loadROM( const char *logical, u8 *dst, u32 expected )
{
...
struct RealMachine
{
	...
	RealMachine()
	{
		...
		haveROMs = loadROM( "kernal.rom", kernal, C64_KERNAL_SIZE )
		        && loadROM( "basic.rom",  basic,  C64_BASIC_SIZE );
		...
		if ( loadROM( "chargen.rom", chargen, C64_CHARROM_SIZE ) )
