...
void CSCPUKernel::Run( void )
{
	...
	// on the HDMI console.
	//
	// Everyone whose code is actually IN the binary is named: Scott Hutter for
	// scpu-emu, Carsten Dachsbacher for the RAD bus timing this is built on,
	// Rene Stange for Circle, which is linked in, and the VICE team, whose
	// behaviour this emulator is verified against and in places transcribed
	// from. Patrick Bass is named for the SCPU/RAMLink fork. GPLv3 asks for
	// the attribution; it costs eight lines and a second of boot.
	logger->Write( "SCPU", LogNotice, "SuperCPU Emulator - Build #%s",
	               SCPU_EMULATOR_BUILD );
	logger->Write( "SCPU", LogNotice, "" );
	logger->Write( "SCPU", LogNotice, "Original author: Scott Hutter / xlar54" );
	logger->Write( "SCPU", LogNotice, "https://github.com/xlar54/scpu-emu" );
	logger->Write( "SCPU", LogNotice, "" );
	logger->Write( "SCPU", LogNotice,
	               "SCPU/RAMLink emulation fork created by "
	               "Patrick Bass / baconsammich" );
	logger->Write( "SCPU", LogNotice,
	               "https://github.com/baconsammich/scpu-emu-ramlink" );
	logger->Write( "SCPU", LogNotice, "" );
	logger->Write( "SCPU", LogNotice,
	               "RAD Expansion Unit developed by Carsten Dachsbacher" );
	logger->Write( "SCPU", LogNotice, "https://github.com/frntc/RAD" );
	logger->Write( "SCPU", LogNotice, "" );
	logger->Write( "SCPU", LogNotice,
	               "Bare-metal environment: Circle, by Rene Stange" );
	logger->Write( "SCPU", LogNotice, "https://github.com/rsta2/circle" );
	logger->Write( "SCPU", LogNotice, "" );
	logger->Write( "SCPU", LogNotice,
