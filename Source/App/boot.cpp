...
// No `handoff` parameter, deliberately.
//
// It used to take one, and the flag means "the bus is live and these counters
// are real" -- a property of the handover. One caller passed s_HDMIExclusive
// instead, so at VIDEO_MODE 0 the END sample of every measurement was marked
// invalid, scpuRuntimeDelta() rejected the pair, and achieved-khz could not be
// computed in the default video mode at all. It reported handoff=0 rather than
// a wrong number, which is why nobody noticed.
//
// The condition was never anything a caller should supply: it is exactly
// "scpu && scpu->hostBus()", which this function can see for itself. Taking
// the argument away is what stops the mistake recurring -- scpuRuntimeDelta()
// is thoroughly tested, but boot.cpp is bare-metal Circle code with no test
// coverage at all, so the only defence available here is an interface that
// cannot be called wrongly.
static SCPURuntimeSample scpuCaptureRuntimeSample( CSuperCPU *scpu )
{
	SCPURuntimeSample s = {};
	s.handoff = scpu && scpu->hostBus();
...
static void scpuBeginRuntimeDiagnostic( CSuperCPU *scpu )
{
	...
	s_RuntimeBaseline = scpuCaptureRuntimeSample( scpu );
}

static SCPURuntimeResult scpuCurrentRuntimeResult( CSuperCPU *scpu )
{
	const SCPURuntimeSample current = scpuCaptureRuntimeSample( scpu );
...
// Put the interpreter benchmark into the file the card already carries.
//
// benchmark65816() has always measured this -- five representative loops in
// ARM cycles per emulated cycle, plus four A53 PMU counters -- but the numbers
// only ever reached the Pi's HDMI console, which is exactly what is
// unavailable when a RAD lives in a cabinet.
//
// The unit is ARM cycles per base 65816 cycle. At a 1GHz A53 a 20MHz 65816 has
// 50 ARM cycles per emulated cycle to spend, so 50 is the number to beat and
// double it is roughly half speed.
//
// noinline, and no local arrays, both deliberately. The first version of this
// was written the obvious way -- a table of {name, value} built on the stack
// and walked in a loop -- and GCC inlined the whole thing into the boot
// function, which is already thousands of lines long and runs with /DMA held.
// That build did not reach a cursor on real hardware while the one before it
// did, and the boot path is the worst place in this project to be clever: a
// fault there leaves the Pi holding the bus and the Commodore dead, with no
// console to say why. Whatever the precise mechanism was, the fix is to stop
// growing that frame. Straight-line calls into a separate function cost
// nothing that matters here -- this runs once, during startup.
static void __attribute__((noinline))
scpuAppendBenchPair( char *dst, u32 capacity, u32 &length,
                     const char *name, u32 value )
{
	scpuLineChar( dst, capacity, length, ' ' );
	scpuLineText( dst, capacity, length, name );
	scpuLineChar( dst, capacity, length, '=' );
	scpuLineDecimal( dst, capacity, length, value );
}

static void __attribute__((noinline))
scpuAppendBenchDiag( char *dst, u32 capacity, u32 &length, CSuperCPU &scpu )
{
	...
	scpuLineText( dst, capacity, length,
	              "bench arm-per-emucycle (20MHz at 1GHz needs <=50):" );
	scpuAppendBenchPair( dst, capacity, length, "bank1", scpu.benchArmPerEmuCycle() );
	scpuAppendBenchPair( dst, capacity, length, "bank0", scpu.m_BenchBank0ArmPerCycle );
	scpuAppendBenchPair( dst, capacity, length, "screen", scpu.m_BenchScreenArmPerCycle );
	scpuAppendBenchPair( dst, capacity, length, "io", scpu.m_BenchIOArmPerCycle );
	scpuAppendBenchPair( dst, capacity, length, "superram", scpu.m_BenchSuperRAMArmPerCycle );
	scpuLineChar( dst, capacity, length, '\r' );
	scpuLineChar( dst, capacity, length, '\n' );

	// The PMU densities are what turn a slow number into a diagnosis: I-cache
	// refills, D-cache refills and branch mispredicts each call for a
	// completely different fix.
	scpuLineText( dst, capacity, length, "bench pmu-per-1k-emucycles:" );
	scpuAppendBenchPair( dst, capacity, length, "instr", scpu.m_BenchInstrPer1k );
	scpuAppendBenchPair( dst, capacity, length, "l1i", scpu.m_BenchL1IRefillPer1k );
	scpuAppendBenchPair( dst, capacity, length, "l1d", scpu.m_BenchL1DRefillPer1k );
	scpuAppendBenchPair( dst, capacity, length, "bmiss", scpu.m_BenchBranchMissPer1k );
	scpuAppendBenchPair( dst, capacity, length, "srstretch", scpu.m_BenchSuperRAMStretchPer1k );
	scpuLineChar( dst, capacity, length, '\r' );
	scpuLineChar( dst, capacity, length, '\n' );
}
...
// The RAMLink's own 64K EPROM. Separate buffer: it is a different device's ROM
// and lives for the whole run, exactly like the accelerator's.
static u8 ramLinkROMBuffer[ RAMLINK_ROM_SIZE ];
...
// RLSIZE_* selector -> megabytes, for log lines only.
static u32 ramLinkCardMB( int selector )
{
	switch ( selector )
	{
	case RLSIZE_1MB:  return 1;
	case RLSIZE_2MB:  return 2;
	case RLSIZE_4MB:  return 4;
	case RLSIZE_8MB:  return 8;
	case RLSIZE_16MB: return 16;
	default:          return 0;
	}
}
...
void scpuBootRun( CLogger *logger )
{
	...
	// RAMLink's EPROM is OPTIONAL and is deliberately not part of the required
	// set below. A missing accelerator ROM means the card cannot do its job; a
	// missing RAMLink ROM just means there is no RAMLink, which is the normal
	// case. Only attempted when one is actually configured.
	if ( cfgRAMLinkSize != RLSIZE_NONE )
	{
		const u32 rlROMSize = loadROMFileSized( logger, SCPU_ROM_DIR "ramlink.rom",
		                                        ramLinkROMBuffer, RAMLINK_ROM_SIZE );
		if ( rlROMSize == RAMLINK_ROM_SIZE )
		{
			superCPU.setRAMLinkROM( ramLinkROMBuffer, rlROMSize );
			logger->Write( "SCPU", LogNotice,
			               "RAMLink: %d MB card, ramlink.rom loaded",
			               ramLinkCardMB( cfgRAMLinkSize ) );
		}
		else
		{
			logger->Write( "SCPU", LogWarning,
			               "RAMLink: %d MB card, but SCPU/ramlink.rom is "
			               "missing or not 64K -- registers only",
			               ramLinkCardMB( cfgRAMLinkSize ) );
		}

		// The RAMCard image, if one is on the card.
		//
		// This has to happen HERE, not in CSuperCPU::init(), and the reason is
		// the next few lines of this function: the FAT driver is released
		// before /DMA is asserted, and it is never brought back. So the card
		// must be allocated and filled while the SD is still mounted. Fitting
		// the same card again in CSuperCPU::init() preserves what is in it --
		// see the note on CRAMLink::init() -- so the image survives.
		//
		// A Pi has no battery, so without this a card the user partitioned
		// comes up blank on every cold start, which makes the whole device
		// close to useless. Reading it back out again at shutdown is the other
		// half and is NOT done: writing to FAT while holding the bus is not
		// safe on this design, so the image is read-only. Changes a program
		// makes to the card last until power off.
		superCPU.ramLink().init( (u8)cfgRAMLinkSize );
		u8 *card = superCPU.ramLink().cardRAM();
		const u32 cardBytes = superCPU.ramLink().cardSizeBytes();
		if ( card && cardBytes )
		{
			u32 imageSize = 0;
			if ( readFile( logger, SCPU_DRIVE, SCPU_ROM_DIR "ramlink.img",
			               card, &imageSize, cardBytes ) && imageSize )
			{
				logger->Write( "SCPU", LogNotice,
				               "RAMLink: ramlink.img loaded, %u of %u KB",
				               imageSize >> 10, cardBytes >> 10 );
				if ( imageSize < cardBytes )
					logger->Write( "SCPU", LogNotice,
					               "RAMLink: image is smaller than the card; "
					               "the remainder is blank" );
			}
			else
			{
				// readFile refuses an oversized file outright rather than
				// truncating, and logs which case it was, so do not claim the
				// file is absent -- it may simply be bigger than the fitted
				// card.
				logger->Write( "SCPU", LogNotice,
				               "RAMLink: no usable ramlink.img (absent, or "
				               "larger than the %u KB card) -- card comes up "
				               "blank and must be formatted", cardBytes >> 10 );
			}
		}
	}
	...
	// from that result, so do not repeat the minute-long ladder on every boot.
	{
		...
		scpuAppendBenchDiag( busDiag, sizeof busDiag, busDiagSize, superCPU );
	...
	// Bracket the run below so the diagnostic file can report what the machine
	// ACHIEVED, not only what five micro-loops can do in isolation.
	//
	// scpuBeginRuntimeDiagnostic() was previously called only when the HDMI
	// picture was being driven, so at VIDEO_MODE 0 -- the default, and the
	// mode anyone diagnosing a problem is told to use -- no baseline was ever
	// taken and achieved-khz could not be computed at all. The sample's
	// "handoff" flag only means the bus is live, so there was never a reason
	// to gate it on the display.
	scpuBeginRuntimeDiagnostic( &superCPU );
	...
	if ( !s_HDMIPictureActive && !startupInterrupted )
	{
		...
		u32 runtimeBusDiagSize =
			radBus.formatSelfTestResults( runtimeBusDiag, sizeof runtimeBusDiag );
		// This is the LAST writer of the diagnostic file before the machine is
		// handed over, so anything appended earlier is discarded here. That had
		// been quietly true of the pi-throttle block for as long as it has
		// existed -- it is built at the first write site and has never survived
		// to the card. Append both here, where it counts.
		scpuAppendRuntimeBusDiag( runtimeBusDiag, sizeof runtimeBusDiag,
		                          runtimeBusDiagSize,
		                          scpuCurrentRuntimeResult( &superCPU ) );
		scpuAppendPiThrottledDiag( runtimeBusDiag, sizeof runtimeBusDiag,
		                           runtimeBusDiagSize );
		scpuAppendBenchDiag( runtimeBusDiag, sizeof runtimeBusDiag,
		                     runtimeBusDiagSize, superCPU );
	...
	// Write the RAMCard back, now that the bus is the C64's again.
	//
	// This is the ONLY moment it is safe and worth doing. /DMA is released, so
	// the emulated machine is stopped and the card cannot change underneath
	// the write; the Pi is about to reboot, so the several seconds an 8MB
	// transfer takes cost nothing; and the C64 is back on its own CPU, so a
	// slow SD operation cannot starve the bus.
	//
	// Three guards, and each one is load-bearing:
	//   - opt-in, because a save replaces an image that may have taken real
	//     effort to build, and the emulated device is newer than the firmware
	//     around it
	//   - only when the guest actually wrote to the card, so an ordinary
	//     session that only READ costs nothing and risks nothing
	//   - via a temporary that is renamed over the target, so a failed or
	//     interrupted save leaves the previous image intact rather than a
	//     truncated one
	if ( cfgRAMLinkPersist && superCPU.ramLink().present()
	     && superCPU.ramLink().cardRAM() && superCPU.ramLink().cardSizeBytes() )
	{
		// cardChanged() compares CONTENT, not whether anything wrote. Booting
		// CMD's DOS touches the card and changes none of it, so a touch-based
		// test would rewrite 8MB every session for nothing.
		if ( !superCPU.ramLink().cardChanged() )
			logger->Write( "SCPU", LogNotice,
			               "RAMLink: card unchanged, not rewriting %s",
			               SCPU_RAMLINK_IMG );
		else
		{
			const u32 bytes = superCPU.ramLink().cardSizeBytes();
			logger->Write( "SCPU", LogNotice,
			               "RAMLink: saving %u KB to %s ...",
			               bytes >> 10, SCPU_RAMLINK_IMG );
			CScopedLoggingIRQs saveIRQs;
			if ( writeFileAtomic( logger, SCPU_DRIVE, SCPU_RAMLINK_IMG,
			                      SCPU_RAMLINK_TMP,
			                      superCPU.ramLink().cardRAM(), bytes ) )
			{
				superCPU.ramLink().noteCardSaved();
				logger->Write( "SCPU", LogNotice, "RAMLink: saved" );
			}
			else
				logger->Write( "SCPU", LogError,
				               "RAMLink: save FAILED -- the previous %s is "
				               "untouched", SCPU_RAMLINK_IMG );
		}
	}
