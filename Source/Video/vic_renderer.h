...
{
public:
	// The renderer is deliberately stateless. Static entry points make that
	// contract explicit and avoid implying that a per-instance state must be
	// initialized or retained between scanline bands.
	...
	static VICRenderMode render( const VICRenderState &state, u8 *pixels,
	                             u32 pitch,
	                             VICRenderCollisions *collisions = 0,
	                             VICRenderSpriteSequencer *sprites = 0 );
	...
	static VICRenderMode renderRows( const VICRenderState &state, u8 *pixels,
