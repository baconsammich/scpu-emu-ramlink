	...
	for ( u32 y = firstRow; y < endRow; y++ )
	{
		     ...
		     && sourceY >= 0 && sourceY < VIC_RENDER_DISPLAY_H )
		{
			...
			for ( u32 col = 0; col < 40; col++ )
			{
				...
				// The parentheses are load-bearing. Pointer arithmetic is
				// left-to-right, so `cellPixels + copyLeft - cellLeft` forms
				// the intermediate pointer `cellPixels + copyLeft` -- and
				// copyLeft is an absolute screen column, routinely 32 or more,
				// against an 8-byte array. The final pointer is always in
				// range, but C++ only defines pointer arithmetic that stays
				// within the object (plus one past the end), so the
				// intermediate is undefined behaviour a compiler is entitled
				// to assume cannot happen. Caught by UBSan on
				// vic_raster_replay_renders_border_splits_at_the_planned_rows.
