...
const char timingNames[TIMING_NAMES][32] = {
	...
	"REUSIZE",
	// Size of the emulated CMD RAMLink's RAMCard. A SELECTOR, not a size:
	// 1 none, 2 1MB, 3 2MB, 4 4MB, 5 8MB, 6 16MB. Same convention as REUSIZE
	// and for the same reason. Fitting one needs SCPU/ramlink.rom; without the
	// image the unit still answers its registers, as an empty socket does.
	// See Source/RAMLink/ramlink.h.
	"RAMLINK_SIZE",
	// Write the RAMCard back to SCPU/ramlink.img when the button is pressed.
	// 1 on, 0 off. Off is the safe default: a save replaces an image you may
	// have spent a long time building, and the emulated device is newer than
	// the rest of this firmware. Nothing is written unless the guest actually
	// wrote to the card, and the save goes via a temporary that is renamed
	// over the target, so a failure leaves the old image intact.
	"RAMLINK_PERSIST"
};
...
// Emulated RAMLink RAMCard size, as an RLSIZE_* selector from
// Source/RAMLink/ramlink.h. Defaults to "no RAMLink fitted".
extern int cfgRAMLinkSize;

// Whether the RAMCard is written back to SCPU/ramlink.img on a button press.
// Set from RAMLINK_PERSIST; defaults off.
extern int cfgRAMLinkPersist;
