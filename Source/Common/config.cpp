...
int cfgRAMLinkSize = RLSIZE_NONE;
int cfgRAMLinkPersist = 0;
...
int readConfig( CLogger *logger, const char *DRIVE, const char *FILENAME )
{
	...
	bool ramLinkSizeSeen = false;
	bool ramLinkPersistSeen = false;
			...
			if ( ptr )
			{
				...
				for ( int i = 0; i < TIMING_NAMES; i++ )
					if ( strcmp( ptr, timingNames[ i ] ) == 0 && ( ptr = strtok_r( 0, "\"", &rest ) ) )
					{
						...
						if ( i == 43 ) ramLinkSizeSeen = true;
						if ( i == 44 ) ramLinkPersistSeen = true;
						while ( *ptr == '\t' || *ptr == ' ' ) ptr++;
					#ifdef DEBUG_OUT
	...
	// Same rule for RAMLINK_SIZE, same reasoning.
	if ( ramLinkSizeSeen )
	{
		cfgRAMLinkSize = timingValues[ 43 ];
		if ( cfgRAMLinkSize < RLSIZE_NONE || cfgRAMLinkSize > RLSIZE_16MB )
			cfgRAMLinkSize = RLSIZE_NONE;
	}

	if ( ramLinkPersistSeen )
		cfgRAMLinkPersist = ( timingValues[ 44 ] != 0 );
