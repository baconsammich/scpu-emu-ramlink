/*
   ...
   The SuperCPU owns one of these and connects it through the interceptor chain
   in reu_wiring.h, preserving the accelerator register handler already
   attached to CC64Memory.
...
private:
	// Owns a heap allocation; shallow copies would double-free it.
	CREU( const CREU & ) = delete;
	CREU &operator=( const CREU & ) = delete;
