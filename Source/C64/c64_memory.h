...
public:
	...
	// The accelerator's hardware-register enable. The memory layer needs it
	// because the DOS extension does two quite different things depending on
	// it -- see CC64Memory::dosExtensionUltimax().
	virtual const bool *hardwareRegsStatePtr() const { return 0; }
...
public:
	...
	void setIOInterceptor( IIOInterceptor *io )
	{
		...
		m_HWRegsState = io ? io->hardwareRegsStatePtr() : 0;
	}
	// The interceptor currently installed. A diagnostic harness that wants to
	// OBSERVE I/O has to wrap this, not the accelerator's register block:
	// wrapping the register block alone silently drops every other device on
	// the chain -- the REU and RAMLink included -- so the machine being
	// observed is not the machine that runs.
	IIOInterceptor *ioInterceptor() const { return m_IO; }
	...
	// --- cartridge KERNAL substitution (CMD RAMLink) ----------------------
	// A RAMLink carries its own C64 KERNAL and serves $E000-$FFFF from it
	// whenever it is installed. That is not an optional extra: the RAMLink DOS
	// wedge -- the thing that makes device 16 exist at all -- lives in that
	// KERNAL. A RAMLink with its registers emulated but its ROM absent is a
	// device that answers probes and can never be opened.
	//
	// `rom` is the whole 64K image; the two KERNAL images inside it are at
	// $8000 (used while RAMLink is ON) and $A000 (while it is off but fitted).
	// `onState` points at the device's live on/off flag -- see onStatePtr().
	//
	// Per VICE's ramlink_romh_read(). Note this is the ONE ROM mapping VICE
	// does not exclude for an SCPU64; $8000-$9FFF and $A000-$BFFF are excluded
	// there because the accelerator has its own.
	void setCartridgeKernal( const u8 *rom, const bool *onState )
	{
		m_CartKernal = rom;
		m_CartKernalOn = onState;
	}

	// Whether this machine maps the cartridge's KERNAL over $E000-$FFFF at all
	// in ordinary operation.
	//
	// On an unaccelerated C64 it does, on or off -- that is how RAMLink's DOS
	// wedge gets into the machine. On a SuperCPU it NEVER does: VICE's scpu64
	// config table has no RH or UM entry for $E000-$FFFF in any configuration
	// with the boot ROM unmapped, so ramlink_romh_read() is simply never
	// reached there. The accelerator's own KERNAL shadow owns that window, and
	// RL-DOS runs out of the accelerator's ROM.
	//
	// This is not a policy choice, it is the hardware. Getting it wrong is not
	// subtle either: CMD's ALT KERNAL does
	//
	//     $FE70  STA $DF7E     RAMLink ON
	//     $FE73  JSR $FE98
	//     $FE76  STA $DF7F     RAMLink OFF
	//
	// and if switching RAMLink on swaps the cartridge's ROM in underneath the
	// program counter, the JSR at $FE73 is fetched from the cartridge instead
	// and the machine leaves for somewhere else entirely -- observed as a boot
	// that reaches $F9D2 where VICE reaches $FE98, and never returns.
	//
	// The one exception is the DOS extension's Ultimax overlay, which is
	// exactly what the `ultimax` argument to cartridgeKernalRead() is for.
	bool m_CartKernalMapped = false;

	// The RAMLink DOS windows at $8000-$9FFF and $A000-$BFFF.
	//
	// VICE maps these from the cartridge whenever RAMLink is fitted, its DOS is
	// mapped in ($DF60/$DF70) and it is enabled -- and excludes them, explicitly
	// and only, for an SCPU64, because the accelerator carries RL-DOS itself.
	// So this is the UNACCELERATED path: without it RAMLink's own KERNAL calls
	// into its DOS, finds ordinary RAM, and the machine never finishes booting.
	void setCartridgeDOS( const bool *dosMapped, const u32 *romBase, bool enable )
	{
		m_CartDOSMapped = dosMapped;
		m_CartROMBase = romBase;
		m_CartDOSEnabled = enable;
	}

	inline bool cartridgeDOSRead( u16 a, u8 &value ) const
	{
		if ( !m_CartDOSEnabled || !m_CartKernal ) return false;
		if ( !m_CartDOSMapped || !*m_CartDOSMapped ) return false;
		const u32 base = m_CartROMBase ? *m_CartROMBase : 0;
		if ( a >= 0x8000 && a <= 0x9FFF )
		{
			if ( c64MapRead( a, m_BankMode ) != REG_RAM ) return false;
			value = m_CartKernal[ ( base | ( a & 0x1FFF ) ) & ( 0x10000 - 1 ) ];
			return true;
		}
		if ( a >= 0xA000 && a <= 0xBFFF )
		{
			value = m_CartKernal[ ( base | 0x2000 | ( a & 0x1FFF ) ) & ( 0x10000 - 1 ) ];
			return true;
		}
		return false;
	}
	bool cartridgeKernalFitted() const { return m_CartKernal != 0; }

	// The two PLA input lines the expansion port drives, in VICE's sense
	// (export.exrom / export.game, where exrom is already inverted). The
	// combination that matters here is Ultimax: game=1, exrom=0. Nothing
	// fitted is game=0, exrom=0, which is the default and disables the
	// overlay entirely -- so a machine with no cartridge is unaffected.
	void setCartridgeLines( bool exrom, bool game )
		{ m_CartExrom = exrom; m_CartGame = game; }

	// Which byte, if any, the cartridge KERNAL supplies for this address.
	// Returns false when the address is not the cartridge's to answer.
	inline bool cartridgeKernalRead( u16 a, u8 &value, bool ultimax = false ) const
	{
		if ( !m_CartKernal || a < 0xE000 )
			return false;

		// Not this machine's window unless the overlay has made it one.
		if ( !ultimax && !m_CartKernalMapped )
			return false;

		// $FF00-$FF0F and $FFF0-$FFFF are excluded from the ON image, and that
		// exclusion IS the bank-switching mechanism rather than a detail.
		// Diffing a 2.01 image against the stock KERNAL shows RAMLink patches
		// EXACTLY those 16 bytes at $FF00-$FF0F: that is its trampoline, placed
		// where it reads the same whichever bank is selected, so the code doing
		// the switch survives the switch. The vectors at $FFF0-$FFFF are there
		// for the same reason.
		//
		// So a hole does NOT fall through to the machine's own KERNAL -- it
		// falls to RAMLink's OFF image, which is where the trampoline lives.
		// Falling through loses it, and the switch becomes a jump into whatever
		// the host KERNAL happens to hold at that address.
		const bool inHole = ( a >= 0xFF00 && a < 0xFF10 ) || ( a >= 0xFFF0 );
		const bool on = m_CartKernalOn && *m_CartKernalOn;

		if ( on && !inHole )
		{
			value = m_CartKernal[ 0x8000 + ( a & 0x1FFF ) ];
			return true;
		}

		// The OFF image. Only where the KERNAL would have been banked in --
		// VICE gates this branch on the CPU port's HIRAM bit.
		if ( c64MapRead( a, m_BankMode ) != REG_KERNAL )
			return false;

		value = m_CartKernal[ 0xA000 + ( a & 0x1FFF ) ];
		return true;
	}

	// The pattern C64 DRAM powers up holding.
	//
	// NOT zero. Real DRAM comes up in bands, and software reads it: CMD's DOS
	// reads uninitialised RAM during its RAMLink setup and acts on what it
	// finds. "Uninitialised" is not the same as "zero", and a machine that
	// returns zero there is not the machine anything was written for.
	//
	// This is VICE's pattern, from its RUNTIME resource defaults rather than
	// the struct initialisers in ram.c, which differ and are not what is used:
	//
	//     RAMInitStartValue   0      RAMInitValueOffset          2
	//     RAMInitValueInvert  4      RAMInitPatternInvert    16384
	//                                RAMInitPatternInvertValue 255
	//
	// giving 4-byte bands offset by two, inverted wholesale every 16KB:
	//
	//     $0B00:  00 00 ff ff ff ff 00 00 00 00 ff ff ff ff 00 00
	//
	// VICE also flips individual bits with 0.1% probability
	// (RAM_INIT_RANDOM_CHANCE_DEFAULT). That is deliberately NOT reproduced:
	// it would make this emulator's boot unrepeatable, and a reproducible
	// power-on state is worth more here than modelling one more decimal place
	// of a real machine's caprice.
	static u8 powerOnRAMByte( u32 addr )
	{
		const u8 band    = ( ( ( addr + 2 ) / 4 ) & 1 ) ? 0xFF : 0x00;
		const u8 pattern = ( ( addr / 16384 ) & 1 ) ? 0xFF : 0x00;
		return (u8)( band ^ pattern );
	}
	...
	{
		// No DOS-extension case here: the extension's bank-1 mapping is
		// READ-ONLY. See dosExtensionMapsBank1().
	...
	// True while the boot ROM is mapped AND the hardware registers are on --
	// the state in which the DOS extension stops being a pair of bank-1
	// windows and becomes something else entirely. See dosExtensionUltimax().
	inline bool bootROMOwnsMap() const
	{
		return m_BootmapActive && m_HWRegsState && *m_HWRegsState;
	}
	...
	inline bool dosExtensionMapsBank1( u16 addr ) const
	{
		...
		//
		// The gate is `!bootROMOwnsMap()`, not `!m_BootmapActive`: VICE's table
		// maps bank 1 here in EVERY configuration except bootmap+hwenable
		// together, where these ranges go to the Ultimax overlay instead.
		//
		// READS ONLY. VICE's R1 entry sets a read function and a read base and
		// leaves the write hook at its preset -- ram_store, which writes bank
		// 0 -- and the comment in scpu64meminit.c says so outright:
		// "/* write hook preset, ram */". So the DOS extension shows bank 1
		// through these windows without making them writable.
		//
		// Routing writes there as well corrupts the thing being run: bank 1
		// $8000-$9FFF IS the RL-DOS code, and CMD's DOS writes to $8000-$9FFF
		// constantly as ordinary RAM. Measured as bank 1 degrading from
		// 8192/8192 matching the ROM to 1555/8192, and the machine then
		// spinning in what was left of it.
		return ( ( addr >= 0x1000 && addr <= 0x5FFF )
		      || ( addr >= 0x8000 && addr <= 0x9FFF ) )
		    && !bootROMOwnsMap() && m_ROMShadow
		    && m_DOSExtState && *m_DOSExtState;
	}

	// The DOS extension's OTHER job: an Ultimax overlay onto the cartridge.
	//
	// Turning the DOS extension on while the boot ROM is mapped does not add a
	// window -- it REPLACES the accelerator's own EPROM with an Ultimax view of
	// whatever is in the expansion port, everywhere except I/O. That is how
	// CMD's boot code gets to look at a RAMLink's ROM at all, and it is the
	// hinge the whole RAMLink path turns on: the boot reads $E0AB this way and
	// demands $7E (ramlink201.bin[$A0AB]) before it will switch a RAMLink on.
	// Get this wrong and the machine boots perfectly, reports no RAMLink, and
	// gives you nothing to go on.
	//
	// Transcribed from VICE's scpu64 config table (scpu64meminit.c): the
	// F8 -> UM transition at $1000-$CFFF and $E000-$FFFF happens exactly when
	// bootmap=1, hwenable=1, game=1, exrom=0 -- i.e. when a cartridge is
	// holding the port in Ultimax. A RAMLink always does (ramlink.c's
	// ramlink_passthrough_changed), which is why it is visible at all.
	inline bool dosExtensionUltimax() const
	{
		return bootROMOwnsMap()
		    && m_CartGame && !m_CartExrom
		    && m_DOSExtState && *m_DOSExtState;
	}
	...
	// CMD RAMLink's own KERNAL, and its live on/off flag. Null when no RAMLink
	// with a ROM is fitted, which is the ordinary case.
	const u8   *m_CartKernal = 0;
	const bool *m_CartKernalOn = 0;
	bool        m_CartExrom = false;
	bool        m_CartGame  = false;
	const bool *m_CartDOSMapped = 0;
	const u32  *m_CartROMBase = 0;
	bool        m_CartDOSEnabled = false;
