	  ...
	  m_TimingHook( 0 ), m_TimingHookCtx( 0 )
{
	...
	for ( u32 i = 0; i < C64_RAM_SIZE; i++ )      m_RAM[ i ] = powerOnRAMByte( i );
...
void CC64Memory::reset()
{
	...
	// Power-on pattern, not zero -- see powerOnRAMByte(). A reset on real
	// hardware does not clear DRAM either; what survives is whatever was
	// there, and the power-on bands are the closest reproducible stand-in.
	for ( u32 i = 0; i < C64_RAM_SIZE; i++ )
		m_RAM[ i ] = powerOnRAMByte( i );
...
u8 CC64Memory::read8( scpu_addr_t addr )
{
	...
	// The DOS extension's Ultimax overlay outranks the boot ROM.
	//
	// While it is on, the accelerator's EPROM is not mapped at all: VICE's
	// table turns every area outside I/O into an Ultimax view of the expansion
	// port. What that AMOUNTS to, measured rather than reasoned about, is
	// remarkably plain -- a RAMLink drives only $E000-$FFFF, and every other
	// Ultimax window ends up back at the machine's ordinary map:
	//
	//     $1000-$9FFF  bank-0 RAM        (VICE: ramlink forces CART_READ_C64MEM)
	//     $A000-$BFFF  the BASIC shadow  (VICE: mem_read_without_ultimax)
	//     $C000-$CFFF  bank-0 RAM
	//     $D000-$DFFF  I/O, untouched
	//     $E000-$FFFF  the RAMLink's own KERNAL image
	//
	// So the whole overlay is: stop mapping the boot ROM, and let the
	// cartridge answer $E000-$FFFF. Everything below simply falls through to
	// the ordinary PLA path, which already produces exactly those answers.
	// (Values probed out of VICE at cfg=$f7 with an 8MB RAMLink attached:
	// $1234->ram0, $6234->ram0, $8234->ram0, $a234->bank1, $c234->ram0,
	// $e0ab->$7e. Writes need nothing: VICE's ultimax stores all end in
	// mem_store_without_ultimax() when a RAMLink is fitted, i.e. RAM, which is
	// what the write path does already.)
	//
	// This is the window CMD's boot code uses to ask "is a RAMLink there?" --
	// it reads $E0AB and demands $7E before it will switch one on.
	const bool ultimaxOverlay = dosExtensionUltimax();

	if ( ultimaxOverlay && a >= 0xE000 )
	{
		u8 cartValue;
		if ( cartridgeKernalRead( a, cartValue, true ) )
		{
			noteBusAccess( a, false );
			return cartValue;
		}
	...
	if ( m_BootmapActive && !ultimaxOverlay
	...
	// A fitted RAMLink serves $E000-$FFFF from its own KERNAL. Checked here,
	// after bootmap -- the accelerator's own boot ROM still outranks it, which
	// is right: CMD's boot code runs before any KERNAL, cartridge or not.
	if ( m_CartKernal && a >= 0xE000 )
	{
		u8 cartValue;
		if ( cartridgeKernalRead( a, cartValue ) )
			return cartValue;
	}
	if ( m_CartDOSEnabled && a >= 0x8000 && a <= 0xBFFF )
	{
		u8 cartValue;
		if ( cartridgeDOSRead( a, cartValue ) )
			return cartValue;
	}
...
void CC64Memory::write8( scpu_addr_t addr, u8 value )
{
	...
	// Deliberately no DOS-extension case: its bank-1 mapping is read-only,
	// and writes through these windows land in ordinary bank-0 RAM.
	// See dosExtensionMapsBank1().
