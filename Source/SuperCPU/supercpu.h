...
#include "../RAMLink/ramlink_wiring.h"
#include "fast_ram.h"
...
public:
	...
	// ramLinkSize is an RLSIZE_* selector. Defaulted to absent so every
	// existing caller compiles unchanged and gets a machine with no RAMLink.
	bool init( IC64Bus *bus, SCPUCoreType core = SCPU_CORE_6502,
	           u32 simmMB = SCPU_SIMM_16MB,
	           u8 reuSize = REUSIZE_NONE,
	           u8 ramLinkSize = RLSIZE_NONE );

	// The RAMLink's 64K EPROM. Call before init(), like the other ROM setters.
	// Without it the unit still answers its registers, exactly as the hardware
	// does with an empty socket.
	void setRAMLinkROM( const u8 *data, u32 length )
	{
		m_RAMLinkROM = data;
		m_RAMLinkROMLength = length;
	}
	...
	CRAMLink           &ramLink()     { return m_RAMLink; }
...
private:
	...
	// RAMLink sits between the accelerator's own registers and the REU in the
	// I/O chain, which is where it sits in a real machine: it claims I/O2 and
	// the REU lives behind it in the RAM-Port.
	CRAMLink            m_RAMLink;
	CRAMLinkInterceptor m_RAMLinkInterceptor;
	CRAMLinkREUBridge   m_RAMLinkREUBridge;
	CRAMLinkFixedClock  m_RAMLinkClock;
	const u8           *m_RAMLinkROM = 0;
	u32                 m_RAMLinkROMLength = 0;
