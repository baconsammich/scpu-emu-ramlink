                      ...
                      u8 reuSize, u8 ramLinkSize )
{
	...
	// RAMLink. Ordered AHEAD of the REU deliberately -- see the note on
	// CIOInterceptorChain. The accelerator registers stay primary because they
	// must observe the $DF7E/$DF7F strobes for $D0BC bit 6 and the vector
	// reroute, and they decline the write so RAMLink still receives it.
	m_RAMLink.init( ramLinkSize );
	if ( m_RAMLinkROM && m_RAMLinkROMLength )
		m_RAMLink.setROM( m_RAMLinkROM, m_RAMLinkROMLength );
	m_RAMLinkREUBridge.attach( &m_REU );
	m_RAMLink.attachREU( &m_RAMLinkREUBridge );
	m_RAMLink.attachClock( &m_RAMLinkClock );
	m_RAMLinkInterceptor.attach( &m_RAMLink );

	// A RAMLink with a ROM also substitutes the machine's KERNAL. Without this
	// the device answers its registers and can never be opened, because the
	// DOS wedge that makes device 16 exist lives in that KERNAL.
	if ( m_RAMLink.present() && m_RAMLink.hasROM() )
	{
		m_Memory.setCartridgeKernal( m_RAMLink.romImage(),
		                             m_RAMLink.onStatePtr() );

		// A fitted RAMLink holds the expansion port in Ultimax and keeps it
		// there -- VICE's ramlink_passthrough_changed() asserts GAME and
		// releases EXROM for anything that is not another Ultimax cartridge
		// behind it. That is a standing line state, not a mode the device
		// switches between, so it is set once here.
		//
		// It matters because the SuperCPU's DOS extension only produces its
		// Ultimax overlay in this line state, and that overlay is the only way
		// CMD's boot code can see the RAMLink's ROM well enough to believe the
		// device is there. See CC64Memory::dosExtensionUltimax().
		m_Memory.setCartridgeLines( /*exrom*/ false, /*game*/ true );
	}

	m_IOChain.attach( &m_Registers, &m_RAMLinkInterceptor, &m_REUInterceptor );
...
void CSuperCPU::reset()
{
	...
	// The accelerator's ALTERNATE KERNAL at bank-1 $6000-$7FFF.
	//
	// The compendium's mirror table is explicit that this is a DIFFERENT image
	// from the one at bank-1 $E000, and that bank-0 $E000-$FFFF is served from
	// it while the hardware registers are OPEN:
	//
	//     $6000-$7FFF  ALT. KERNAL  $E000-$FFFF  CPU H/W registers enabled
	//
	// Seeding it with the C64's own KERNAL, as this used to, is wrong in a way
	// that matters: ROM $7F00-$7F0F is a jump table -- JMP $1B5E / $1B76 /
	// $1BB2 -- which appears at KERNAL address $FF00-$FF0F once the registers
	// are open, and those targets are the accelerator's RAMLink code. With the
	// C64 KERNAL there instead, opening the registers gets the C64's $FF00
	// bytes and the route into that code does not exist.
	if ( m_MemoryMap.romImage() && m_MemoryMap.romLength() >= 0x8000 )
		for ( u32 i = 0; i < 0x2000; i++ )
			m_MemoryMap.m_Bank1[ 0x6000 + i ] = m_MemoryMap.romImage()[ 0x6000 + i ];

	// RL-DOS is TWO windows, not one. The same table:
	//
	//     $1000-$5FFF  RLDOS  $1000-$5FFF  CPU DOS extensions enabled
	//     $8000-$9FFF  RLDOS  $8000-$9FFF  CPU DOS extensions enabled
	//
	// 20KB plus 8KB. dosExtensionMapsBank1() already maps both; only the
	// second was being filled, so the jump table's targets at $1B5E/$1B76/
	// $1BB2 landed in empty SRAM.
	if ( m_MemoryMap.romImage() && m_MemoryMap.romLength() >= 0x6000 )
		for ( u32 i = 0x1000; i < 0x6000; i++ )
			m_MemoryMap.m_Bank1[ i ] = m_MemoryMap.romImage()[ i ];

	// RL-DOS at bank-1 $8000-$9FFF.
	//
	// This is the RAMLink support, and it belongs to the ACCELERATOR, not to
	// the RAMLink. Two pieces of CMD's own documentation say so directly: the
	// compendium's mirror table names this exact window
	//
	//     $8000-$9FFF  RLDOS  read only  CPU DOS extensions enabled
	//
	// and the SuperCPU 128 V2 manual's startup error #05 is "RL-DOS ROM
	// Checksum Error" -- so RL-DOS is part of the accelerator's ROM and is
	// verified at power-on. VICE agrees from the other side: it excludes
	// RAMLink's own $8000-$9FFF and $A000-$BFFF mappings for an SCPU64,
	// explicitly, because the accelerator carries this instead.
	//
	// Seeded from the accelerator's ROM image at the same offset, exactly as
	// BASIC and the KERNAL above are, and for the same reason: the window has
	// to hold something before any code reaches through the DOS extension. The
	// ROM's first 64K mirrors the bank-1 layout -- $8000 RL-DOS, $A000 BASIC,
	// $E000 KERNAL -- which is why the offsets match.
	if ( m_MemoryMap.romImage() && m_MemoryMap.romLength() >= 0xA000 )
		for ( u32 i = 0; i < 0x2000; i++ )
			m_MemoryMap.m_Bank1[ 0x8000 + i ] = m_MemoryMap.romImage()[ 0x8000 + i ];
