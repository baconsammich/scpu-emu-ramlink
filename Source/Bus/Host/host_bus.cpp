...
u8 CHostBus::read( u16 addr )
{
	...
	{
		// I/O1 and I/O2 are not memory. Those two pages are driven only when a
		// cartridge answers, and every device this stack models is reached
		// through IIOInterceptor long before the bus -- so a read arriving
		// HERE is by definition an address nothing is decoding, i.e. an
		// unterminated bus. A real C64 returns whatever the VIC last fetched;
		// it does NOT return the byte last written there.
		//
		// The high address byte is the usual open-bus stand-in. The
		// distinction is load-bearing rather than cosmetic: CMD's DOS probes
		// for an REU by writing $DF02-$DF05 and reading them back --
		//
		//     $4FBF  STA $DF02,X   (x4)
		//     $4FC7  CMP $DF02,X   (x4)   mismatch -> not present
		//
		// -- and a plain array passes that test 256 times over, inventing an
		// REU that is not plugged in and sending the boot down a path the
